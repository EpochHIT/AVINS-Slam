...
rgb_inactive_pcd (new RgbPointCloud)
{
    ...
    // message sync tolerance (seconds) for matching image/depth to pose
    // default: 10ms, can be overridden by ROS param "msg_sync_tolerance"
    if (!nh.getParam("msg_sync_tolerance", msg_sync_tolerance))
        msg_sync_tolerance = 0.01;
...
void SurfelMap::synchronize_msgs()
{
    ...
    for(int scan_pose = 0; scan_pose < pose_reference_buffer.size(); scan_pose++)
    {
        ...
        // 查找时间戳匹配（默认10ms，参数msg_sync_tolerance可调）
        for(int image_i = 0; image_i < image_buffer.size(); image_i++)
        {
            ...
            if(time_diff < msg_sync_tolerance)
            {   
                image_num = image_i;
                break;  // 找到匹配的就退出
            }
        ...
        for(int depth_i = 0; depth_i < depth_buffer.size(); depth_i++)
        {
            ...
            double time_diff = fabs(depth_time - pose_reference_time);
        ...
        if( image_num < 0 || depth_num < 0) {
            ROS_WARN_THROTTLE(1.0, "No matching synchronized data found for pose at time %.6f", pose_reference_time);
            continue;
        }
